...
def create_dictionary(input_csv):
    ...
    for item in dictionary:
        csv_dict[item[vars[0]]][item[vars[5]]] = {}

        for var in vars[1:]:
            if var == vars[5]:
                continue
            if item[var] == 'L':
                csv_dict[item[vars[0]]].pop(item[vars[5]])
            else:
                csv_dict[item[vars[0]]][item[vars[5]]][var] = item[var]
...
if __name__ == "__main__":
    ...
    with open(f"{INPUT_NAME}_2.json", 'w') as fp:
